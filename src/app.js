...
function boot() {
  ...
  function cursorState() { const definition = selectedDefinition(), occupied = run?.structures?.some((building) => building.col === cursor.col && building.row === cursor.row), affordable = (run?.budget || 0) >= (definition?.cost || Infinity); return { col: cursor.col, row: cursor.row, type: selectedTool, valid: !occupied && affordable }; }
  ...
  function renderPlay() {
    ...
    if (storm) {
      ...
      document.getElementById('cursor-status').textContent = (tool?.name || selectedTool) + ' - ' + (tool?.cost || 0) + ' \uBAA9\uC7AC - ' + (state.valid ? '\uBC30\uCE58 \uAC00\uB2A5' : state.valid === false ? '\uC0AC\uC6A9 \uC911\uC774\uAC70\uB098 \uBAA9\uC7AC \uBD80\uC871' : '\uC120\uD0DD');
  ...
  function placeCurrent() { if (!run || run.status !== 'prepare') return; try { actionEvents(placeBuilding(run, selectedTool, cursor.col, cursor.row)); } catch (error) { console.error(error); } renderPlay(); }
